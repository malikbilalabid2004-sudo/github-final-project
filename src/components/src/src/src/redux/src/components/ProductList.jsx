...
export default function ProductList({ onHomeClick, onCartClick }) {
  ...
  return (
    <div className="app-container">
      <Header view="products" onHomeClick={onHomeClick} onProductsClick={() => {}} onCartClick={onCartClick} />
      <main className="product-list">
        <div className="product-list-intro">
          <h1>The Catalog</h1>
          <p>Nine plants, three ways to fill a room. Everything ships hand-checked and ready to pot.</p>
        </div>

        {categories.map((category) => (
          <section key={category.id} className="category-section" aria-labelledby={`${category.id}-heading`}>
